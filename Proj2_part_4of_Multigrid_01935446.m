%Q4, Poisson equation, SOR method on multigrids
%dx=dy=h!
clear
q=10;
s=10;
r=8;
h_f=0.05;%The fine mesh
h_c=0.1%The corse mesh
L=((s+q)/h_f);
N=(r/h_f);
phi= zeros(N+1,L+1);
%i=2:L;
...
while residual_new>=tolerance;
    ...
    for i2=2:1:L  
         ...
         %free (-q,0] & (1,s)
         if (-q+((i2-1)*h_f))<=0 | (-q+((i2-1)*h_f))>1
             phi(1,i2)=0.25*w*(phi(1,i2+1)+phi(1,i2-1)+2*phi(2,i2))+(1-w)*phi_old(1,i2);
                %boundary layer (1,0)
         else
             phi(1,i2)=0.25*w*(phi(1,i2+1)+phi(1,i2-1)+2*phi(2,i2)-((2*h_f+phi(1,i2+1)-phi(1,i2-1))*(d_y((-q+((i2-1)*h_f))))))+(1-w)*phi_old(1,i2);
         end;
    end
    for j=2:N
        for i=2:L
    ...
    %sweep here 
    r_f=abs(phi-phi_old);%define the residual on the fine mesh
    r_c=restrict(r_f);%apply the restriction
    z_f=interpolate(r_c);%place the error back to the fine mesh
    phi=phi+z_f;%apply the adjustment
...
plot(residual);
%hold on
%waterfall(-q:h_f:s,0:h_f:r,phi)
fprintf('Number of iteration to reach residual -7')
n
%uncomment the following line to verify the solution
%p1_ascertain(phi,h_f,q,s)
function y=d_y(x)
tau=0.05;
A=0.298222773;
B=0.127125232;
