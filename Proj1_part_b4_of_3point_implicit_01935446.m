...
%Assembly the stiffness matrix A for fully implicit method first
for n=1:1:(L/h)-1
    C=[];%a row vector
    for j=1:1:(L/h)-1
...
%use implicit method once to approximate row no.2, ie t=k
U_temp=tridiag(A,(U(:,1)))';
U_temp_boundary=[0;U_temp;0];%add boundary condition
U=[U,U_temp];
U_boundary=[U_boundary, U_temp_boundary];

%rewrite new matrix A
A=[];
...
error_implicit=U_boundary(:,end)-C_exact'
%plot(0:h:L,error_implicit')
%hold on
